## test_that("left assign = {}", {
##   code <- "f <- function(x) { y = if (x>0) 1 else 2; y }"
##   cc <- do_code_coverage(code, "f(1)")
##   expect_equal(cc$counters, c("y = if (x>0) 1 else 2", "x>0", "1", "2", "y"))
##   expect_equal(cc$branch_counters, c("1", "2"))
## })

## test_that("left assign =", {
##   code <- "f <- function(x) y = if (x>0) 1 else 2"
##   cc <- do_code_coverage(code, "f(1)")
##   expect_equal(cc$counters, c("y = if (x>0) 1 else 2", "x>0", "1", "2"))
##   expect_equal(cc$branch_counters, c("1", "2"))
## })

test_that("right assign", {
  code <- "f <- function() (if (TRUE) 1 else 2) -> x"
  cc <- do_code_coverage(code, "f()")
  browser()
  expect_equal(cc$counters, c("x <- if (TRUE) 1 else 2", "TRUE", "1", "2"))
  ## expect_equal(cc$branch_counters, c("1", "2"))
})

## test_that("left assign", {
##   code <- "f <- function() x <- if (TRUE) 1 else 2"
##   cc <- do_code_coverage(code, "f()")
##   expect_equal(cc$counters, c("x <- if (TRUE) 1 else 2", "TRUE", "1", "2"))
##   expect_equal(cc$branch_counters, c("1", "2"))
## })
